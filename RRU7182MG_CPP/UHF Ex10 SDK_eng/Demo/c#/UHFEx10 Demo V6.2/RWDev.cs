...
namespace UHF
{
    ...
    {
        private const string DLLNAME = @"UHFEx10.dll";
        ...
        [DllImport(DLLNAME, CallingConvention = CallingConvention.StdCall)]
        public static extern int ExtSetRegion(ref byte ComAdr,
            byte flag,byte band, byte dmaxfre, byte dminfre, int frmComPortindex);

        [DllImport(DLLNAME, CallingConvention = CallingConvention.StdCall)]
        public static extern int ExtGetRegion(ref byte ComAdr,
            ref byte band, ref byte dmaxfre, ref byte dminfre, int frmComPortindex);
        ...
        [DllImport(DLLNAME, CallingConvention = CallingConvention.StdCall)]
        public static extern int Authenticate_G2(ref byte ComAdr, byte[] epc, byte ENum, byte sendRep, byte IncrepLen, byte CSI,
                                            byte MsgLen, byte[] MsgData, byte[] Password,
                                            byte MaskMem, byte[] MaskAdr, byte MaskLen, byte[] MaskData,
                                            ref byte Auth_len, byte[] Auth_data, ref int errorcode,
                                            int frmComPortindex);
        /// <summary>
        /// 16进制数组字符串转换
        /// </summary>
