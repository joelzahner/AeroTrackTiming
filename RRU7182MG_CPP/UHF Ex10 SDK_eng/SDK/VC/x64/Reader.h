...
typedef struct ReadMemParam
{
 unsigned char ReadMem;
 unsigned char ReadAdr[2];
...
public:
	...
	int GetWifiDataFromPort(unsigned char *Recv, int *pLengthOfRecved);
	...
	int GetModuleDetails(unsigned char *address,
							  unsigned char *Version,unsigned char *mType,unsigned char *Tr_Type);
	...
	int InventoryExtMix_G2(unsigned char *address,
		unsigned char QValue,
		unsigned char Session,
	...
	int ExtGetRegion(unsigned char *address,
		unsigned char *Band,
		unsigned char *dmaxfre,
		unsigned char *dminfre);

	int ExtSetRegion(unsigned char *address,
		unsigned char FLAG,
		unsigned char Band,
		unsigned char dmaxfre,
		unsigned char dminfre);
	...
	int OptAntenna(unsigned char *ComAdr,
									   unsigned char opt,
									   unsigned char SetOnce,
									   unsigned char *AntCfg);
	...
	int Inventory_Temperature_ltu32(unsigned char *address,
		unsigned char ENum,
		unsigned char *EPC,
		unsigned char SelTarget,
		unsigned char SelAction,
	...
	int Inventory_Temperature_Ast010(unsigned char *address,
		unsigned char QValue,
		unsigned char Session,
		unsigned char MaskMem,
		unsigned char *MaskAdr,
	...
	int GetBxTagTemperature(unsigned char *ComAdr,
		              unsigned char ENum, 
					  unsigned char *EPC, 
	...
	int ON_GetTempData(unsigned char *address,
		unsigned char *EPC,
		unsigned char ENum,
		unsigned char *Password,
		unsigned char MaskMem,
	...
	int BX_LED_Controll(unsigned char *address,
		unsigned char *EPC,
		unsigned char ENum,
		unsigned char CarrTime,
		unsigned char *Password,
	...
	int SelectCMDByMutipleAntenna(unsigned char *address,
	...
	int SetCalibrationPowerType(unsigned char *address,unsigned char powerType);
	int GetModuleDescribe(unsigned char *address,unsigned char *describeInfor);

	int SetRegionType(unsigned char *address,unsigned char regionType);

	int GetInventory_FindEPC_DataFromPort(unsigned char *Recv, int *pLengthOfRecved,int cmd);
	int Find_epc_list_cfg(unsigned char *address,unsigned char opt,unsigned char CardNum,unsigned char *epc,int epclen);
	int Find_epc_by_list(unsigned char *address,unsigned char Fixed_Q,int Antenna,int *CardNum,unsigned char *epc,int *epclen);

	int SetTempSelect(unsigned char *address,unsigned char TempSelect);

	int MutipleAntennaSwitch(unsigned char *address,unsigned char TargetAddr,unsigned char antenna);
	int SetTriggerLinkageParam(unsigned char *address,unsigned char Action,unsigned char mode,unsigned char mtime,unsigned char RFU,
		unsigned char Union,unsigned char Condition1,unsigned char Condition2);
	int GetTriggerLinkageParam(unsigned char *address,unsigned char Action,unsigned char *mode,unsigned char *mtime,unsigned char *RFU,
		unsigned char *Union,unsigned char *Condition1,unsigned char *Condition2);

	int SetMacAddr(unsigned char *address,unsigned char* mac);
	int GetMacAddr(unsigned char *address,unsigned char* mac);
	int ResetReader(unsigned char *address);

	int Protected_mode_G2(unsigned char *address,
		unsigned char *EPC,
		unsigned char ENum,
	...
	int Authenticate_G2(unsigned char *address,
		unsigned char *EPC,
		unsigned char ENum,
	...
	int SetGPOParam(unsigned char *address,
		unsigned char Enableflag,
		unsigned char ActiveTime);

	int GetGPOParam(unsigned char *address,
		unsigned char* ActiveTime,
		unsigned char* OverturnCnt);

	int SearchWIFI(unsigned char *address,unsigned char *APInfo,int* length,int* Count);

	int SetRJ45MacAddr(unsigned char *address,unsigned char *mac);
	int GetRJ45MacAddr(unsigned char *address,unsigned char *mac);
	int GetWIFIMacAddr(unsigned char *address,unsigned char *mac);
	int GetBluetoothMacAddr(unsigned char *address,unsigned char *mac);
	int GetBleMacAddr(unsigned char *address,unsigned char *mac);
	int RelaysControll(unsigned char *address,unsigned char num,unsigned char status,unsigned char ntime);
	int ReadRJ45ConnectStatus(unsigned char *address,unsigned char *rj45state,unsigned char *localIP,unsigned char *subnet,unsigned char *gatewayIp);
	int ReadWIFIConnectStatus(unsigned char *address,unsigned char *WIFIstate,unsigned char *localIP,unsigned char *subnet,unsigned char *gatewayIp);

	int ChangeToUpdateMode(unsigned char *ComAdr); 
	int WriteBinData(unsigned char *ComAdr,unsigned char Index,unsigned char len,unsigned char *data); 
	int CheckCRC32(unsigned char *ComAdr,unsigned char* crc32);
	int WriteMark(unsigned char *ComAdr);
	int SetWieGandParam(unsigned char *ComAdr,unsigned char WgCfg,unsigned char DataInvl,unsigned char PulWidth,unsigned char PulsInvl,unsigned char OffsetAdr);
	int GetWieGandParam(unsigned char *ComAdr,unsigned char *WgCfg,unsigned char *DataInvl,unsigned char *PulWidth,unsigned char *PulsInvl,unsigned char *OffsetAdr);
	
	int SetFilterParam(unsigned char *ComAdr,unsigned char FilterTime,unsigned char MinReadCount,unsigned char MinRssi);
	int GetFilterParam(unsigned char *ComAdr,unsigned char *FilterTime,unsigned char *MinReadCount,unsigned char *MinRssi);

	int SetCustomFilterParam(unsigned char *ComAdr,unsigned char FliterEn,unsigned char* UserMask);
	int GetCustomFilterParam(unsigned char *ComAdr,unsigned char* FliterEn,unsigned char* UserMask);

	int SetCustomTimeParam(unsigned char *ComAdr,unsigned char FliterTime,unsigned char ReadUserMemLen);
	int GetCustomTimeParam(unsigned char *ComAdr,unsigned char *FliterTime,unsigned char *ReadUserMemLen);
